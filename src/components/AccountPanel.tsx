...
import { X, UserRound, History, Bookmark, LogOut, Briefcase } from 'lucide-react';
import { useAuth } from '../lib/auth';
import { fetchAccountDashboard } from '../lib/api';
import { usePreferences } from '../lib/preferences';

interface Props { open: boolean; onClose: () => void; }
interface DashboardData { profile: any; history: any[]; applications: any[]; }

export default function AccountPanel({ open, onClose }: Props) {
  const { user, logout } = useAuth();
  const { t } = usePreferences();
  const [data, setData] = useState<DashboardData>({ profile: null, history: [], applications: [] });
      ...
      <div className="max-h-[88vh] w-full max-w-3xl overflow-y-auto rounded-[2rem] bg-cream p-5 shadow-2xl sm:p-7" onMouseDown={(e)=>e.stopPropagation()}>
        ...
        {busy ? <p className="mt-6 text-sm text-ink/55">Loading saved data…</p> : <div className="mt-6 grid gap-4 md:grid-cols-2">
          <section className="rounded-3xl border border-ink/10 bg-white p-5">
            <div className="flex items-center gap-2 font-bold"><UserRound size={17} className="text-teal"/> Saved profile</div>
            <div className="mt-3 text-xs"><div className="rounded-xl bg-black/[0.03] p-3"><strong>Entrepreneur / Business</strong><br/>{data.profile?.entrepreneur?'Saved':'Not saved yet'}</div></div>
          </section>
          <section className="rounded-3xl border border-ink/10 bg-white p-5">
            <div className="flex items-center gap-2 font-bold"><History size={17} className="text-teal"/> Match history</div>
            <p className="mt-1 text-xs text-ink/50">{data.history.length} saved match run{data.history.length===1?'':'s'}</p>
            <div className="mt-3 space-y-2">{data.history.slice(0,4).map((h)=><div key={h._id} className="flex items-center justify-between rounded-xl bg-black/[0.03] p-3 text-xs"><span className="flex items-center gap-2"><Briefcase size={14}/> Business match</span><span className="text-ink/45">{new Date(h.createdAt).toLocaleDateString()}</span></div>)}{!data.history.length&&<p className="text-xs text-ink/45">Run a match to create history.</p>}</div>
          </section>
          <section className="rounded-3xl border border-ink/10 bg-white p-5 md:col-span-2">
            <div className="flex items-center gap-2 font-bold"><Bookmark size={17} className="text-teal"/> Saved opportunities</div>
            <div className="mt-3 grid gap-2 sm:grid-cols-2 text-xs">{data.applications.map((a)=><div key={a._id} className="rounded-xl bg-black/[0.03] p-3"><strong>{a.scheme?.shortName||a.scheme?.name||'Scheme'}</strong><br/><span className="text-ink/50">Business • {a.status}</span></div>)}{!data.applications.length&&<p className="text-ink/45">Save an opportunity from your results.</p>}</div>
          </section>
        </div>}
      </div>
    </div>
