...
const HINGLISH_HINTS = /\b(kya|kaise|mujhe|mera|meri|hum|aap|apna|chahiye|karna|karo|batao|bata|loan|scheme|apply|documents?|eligibility|paisa|kitna|kaun|kaha|kyu|hai|hain|nahi|nhi|wala|wali)\b/i;
...
function initialText(language: Language) {
  if (language === 'hi') return 'नमस्ते! मैं Udaan AI हूँ। बिज़नेस योजनाएँ, लोन, eligibility, documents, EMI या आवेदन प्रक्रिया के बारे में पूछिए।';
  if (language === 'hinglish') return 'Namaste! Main Udaan AI hoon. Business schemes, loans, eligibility, documents, EMI ya apply process ke baare me pucho.';
  return 'Hi! I’m Udaan AI. Ask me about business schemes, loans, eligibility, documents, EMI, or how to apply.';
}
...
function localFallback(q: string, language: Language) {
  ...
  if (/document|dastavez|दस्तावेज/.test(l)) return language === 'hi' ? 'आम तौर पर Aadhaar, income certificate, category certificate (यदि लागू हो), business/project documents मांगे जा सकते हैं। Exact checklist selected scheme के official portal पर verify करें।' : language === 'hinglish' ? 'Usually Aadhaar, income certificate, category certificate (if applicable), business/project documents lag sakte hain. Exact checklist selected scheme ke official portal par verify karo.' : 'Common documents can include Aadhaar, income certificate, category certificate if applicable, business/project documents. Verify the exact checklist on the selected official portal.';
...
export default function ChatWidget() {
  ...
  const quickPrompts = useMemo(() => uiLanguage === 'hi'
    ? ['मेरे लिए कौन सी योजना है?', 'कौन से documents चाहिए?', 'MUDRA कैसे apply करें?']
    : uiLanguage === 'hinglish'
      ? ['Mere liye kaunsi scheme?', 'Kaunse documents chahiye?', 'MUDRA kaise apply karu?']
      : ['Which scheme fits me?', 'What documents do I need?', 'How do I apply for MUDRA?'], [uiLanguage]);
        ...
        {open && (
          <motion.div initial={{ opacity: 0, y: 20, scale: .96 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: .96 }} className="fixed bottom-24 right-4 z-[80] flex h-[34rem] w-[24rem] max-w-[94vw] flex-col overflow-hidden rounded-[1.75rem] border border-[#e8b347]/20 bg-[#0b2d25] text-[#f7f0e2] shadow-2xl">
            <div className="flex items-center justify-between border-b border-[#e8b347]/15 bg-[#08231d] px-4 py-3.5 text-[#f7f0e2]">
              <div className="flex items-center gap-2.5"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#e8b347] text-[#08231d]"><Bot size={18}/></span><div><p className="text-sm font-extrabold">Udaan AI</p><p className="text-[10px] text-cream/60">Business schemes · Loans · Application guide</p></div></div>
